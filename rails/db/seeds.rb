...
CSV.foreach('db/csv/companies_master.csv', encoding: 'UTF-8') do |row|
  ...
  office = Office.create(company_id: company.id, ieul_id: row[2], ieul_office_id: row[3],
  ...
  available_area_ids.each do |available_area_id|
    AssessableArea.find_or_create_by!(city_id: available_area_id, office_id: office.id)
  end
